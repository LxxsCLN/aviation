...
const NavBar = ({ locale }) => {
  ...
  const handleThemeChange = () => {
    changeTheme(theme === "night" ? "light" : "night")
  }
    ...
    <div className="navbar bg-base-100">
      <div className="navbar-start">
        <div className="dropdown">
          ...
          <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52">
            ...
            <li><Link href="/flights">{t("Flights")}</Link></li>
            <li><Link href="/shop">{t("GiftShop")}</Link></li>
            <li><Link href="/news">{t("News")}</Link></li>
            <li><Link href="/blog">{t("Blog")}</Link></li>
            <li><Link href="/about">{t("AboutUs")}</Link></li>
            <li><Link href="/contact">{t("Contact")}</Link></li>
            <li onClick={handleLanguageChange}><a>{t("ChangeLanguage")/* add language icon */}</a></li>
            <li className='flex flex-row items-center justify-start'>
              <label className="swap swap-rotate">
                <input id='darkmode' type="checkbox" onChange={handleThemeChange} />
              ...
              <label htmlFor='darkmode' className='px-0'>{theme === "light" ? t("LightMode") : t("DarkMode")}</label>
            </li>
          </ul>
        </div>
        <Link className="text-xl btn btn-ghost" href="/">aviation</Link>
      </div>
      ...
      <div className="navbar-end">
        <ul className="hidden px-1 menu menu-horizontal lg:flex">
          <li onClick={handleLanguageChange}><a className='font-bold'>{t("ChangeLanguage")/* add language icon */}</a></li>
          <label className="swap swap-rotate">
